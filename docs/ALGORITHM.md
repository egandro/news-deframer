...
### Feed Sources
While any valid RSS 2.0 feed is supported (e.g., `http://rss.cnn.com/rss/edition.rss`), the system can also process feeds generated by third-party aggregator services. These services, such as `https://rss.app`, can create RSS feeds for web pages that do not offer them natively.
...
```bash
ORIGINAL_URL=http://rss.cnn.com/rss/edition.rss
